...
public class VisitsAppServiceTest {
    ...
    public void createVisits_VisitsIsNotNullAndVisitsDoesNotExist_StoreVisits() { 
       ...
       CreateVisitsInput visits = new CreateVisitsInput();
        ...
        visits.setVetId(Integer.valueOf(ID.intValue()));
...
//		search.setOperator("equals");
   ...
   //Vets
	@Test
	public void GetVets_IfVisitsIdAndVetsIdIsNotNullAndVisitsExists_ReturnVets() {
		VisitsEntity visits = mock(VisitsEntity.class);
		VetsEntity vets = mock(VetsEntity.class);

		Mockito.when(_visitsManager.findById(any(Integer.class))).thenReturn(visits);
		Mockito.when(_visitsManager.getVets(any(Integer.class))).thenReturn(vets);
		Assertions.assertThat(_appService.getVets(ID)).isEqualTo(_mapper.vetsEntityToGetVetsOutput(vets, visits));
	}

	@Test 
	public void GetVets_IfVisitsIdAndVetsIdIsNotNullAndVisitsDoesNotExist_ReturnNull() {
		Mockito.when(_visitsManager.findById(any(Integer.class))).thenReturn(null);
		Assertions.assertThat(_appService.getVets(ID)).isEqualTo(null);
	}
