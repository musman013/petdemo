...
public class VisitsControllerTest {
	...
	public void setup() {
		...
		final VisitsController visitsController = new VisitsController(visitsAppService,petsAppService,vetsAppService,
	logHelper);
	...
	@Test
	public void CreateVisits_vetsDoesNotExists_ThrowEntityNotFoundException() throws Exception {
	...
	public void GetVets_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() throws Exception {
	    ...
	    mvc.perform(get("/visits/111/vets")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isNotFound());
	...
	public void GetVets_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/visits/" + visits.getId()+ "/vets")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
