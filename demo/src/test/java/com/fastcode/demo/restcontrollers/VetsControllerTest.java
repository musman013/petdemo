...
public class VetsControllerTest {
	...
	public void setup() {
		...
		final VetsController vetsController = new VetsController(vetsAppService,vetSpecialtiesAppService,visitsAppService,
	logHelper);
	...
	public void GetVisits_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		Mockito.when(vetsAppService.parseVisitsJoinColumn("vetId")).thenReturn(joinCol);
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/vets/1/visits?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property abc not found!"));
	...
	public void GetVisits_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(vetsAppService.parseVisitsJoinColumn("vetId")).thenReturn(joinCol);
		mvc.perform(get("/vets/1/visits?search=vetId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetVisits_searchIsNotEmpty() throws Exception {
		...
		Mockito.when(vetsAppService.parseVisitsJoinColumn(anyString())).thenReturn(null);
		mvc.perform(get("/vets/1/visits?search=vetid[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isNotFound());
	}    
