...
public class PetsAppService implements IPetsAppService {
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public UpdatePetsOutput update(Long  petsId, UpdatePetsInput input) {
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public void delete(Long petsId) {

		PetsEntity existing = _petsManager.findById(petsId) ; 
		
		TypesEntity types = existing.getTypes();
		types.removePets(existing);
 
		OwnersEntity owners = existing.getOwners();
		owners.removePets(existing);
 
		_petsManager.delete(existing);
	}
	
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public FindPetsByIdOutput findById(Long petsId) {
    ...
    @Transactional(propagation = Propagation.NOT_SUPPORTED)
	public GetTypesOutput getTypes(Long petsId) {
    ...
    @Transactional(propagation = Propagation.NOT_SUPPORTED)
	public GetOwnersOutput getOwners(Long petsId) {
	...
	public BooleanBuilder searchKeyValuePair(QPetsEntity pets, Map<String,SearchFields> map,Map<String,String> joinColumns) {
        ...
        if(joinCol != null && joinCol.getKey().equals("typeId")) {
		    builder.and(pets.types.id.eq(Long.parseLong(joinCol.getValue())));
		}
        }
		for (Map.Entry<String, String> joinCol : joinColumns.entrySet()) {
