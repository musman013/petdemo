...
import com.fastcode.demo.domain.visits.IVisitsManager;
...
public class VisitsAppService implements IVisitsAppService {
    ...
    //Vets
	// ReST API Call - GET /visits/1/vets
    @Transactional(propagation = Propagation.NOT_SUPPORTED)
	public GetVetsOutput getVets(Integer visitsId) {
		...
		VetsEntity re = _visitsManager.getVets(visitsId);
		return mapper.vetsEntityToGetVetsOutput(re, foundVisits);
	}
	...
	public BooleanBuilder searchKeyValuePair(QVisitsEntity visits, Map<String,SearchFields> map,Map<String,String> joinColumns) {
        ...
        if(joinCol != null && joinCol.getKey().equals("vetId")) {
		    builder.and(visits.vets.id.eq(Integer.parseInt(joinCol.getValue())));
		}
        }
		return builder;
	}
