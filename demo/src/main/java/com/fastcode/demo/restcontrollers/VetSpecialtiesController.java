...
import com.fastcode.demo.application.specialties.SpecialtiesAppService;
...
public class VetSpecialtiesController {
	...
	Optional.ofNullable(vetspecialtiesid).orElseThrow(() -> new EntityNotFoundException(String.format("Invalid id=%s", id)));
		
	FindVetSpecialtiesByIdOutput output = _vetSpecialtiesAppService.findById(vetspecialtiesid);
    Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a vetSpecialties with a id=%s", id)));
	...
	public ResponseEntity<UpdateVetSpecialtiesOutput> update(@PathVariable String id, @RequestBody @Valid UpdateVetSpecialtiesInput vetSpecialties) {
		...
		Optional.ofNullable(currentVetSpecialties).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. VetSpecialties with id=%s not found.", id)));
		
		
		vetSpecialties.setVersion(currentVetSpecialties.getVersion());
