...
import { IInvoices, InvoicesService, InvoicesDetailsComponent, InvoicesListComponent, InvoicesNewComponent } from './index';

describe('InvoicesListComponent', () => {
  let fixture:ComponentFixture<InvoicesListComponent>;
  let component:InvoicesListComponent;
  let el: HTMLElement;
  let constData:IInvoices[] = [
    {   
    ...
    {   
      ...
      petsDescriptiveField: 'name1',
      vetId: 2,
      vetsDescriptiveField: 2,
    },
  ];
  let data: IInvoices[] = [... constData];
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(location.path()).toBe(`/invoices/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`);
    });
